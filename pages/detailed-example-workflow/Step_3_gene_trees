...
1. Construct gene trees:
    ...
    - a file sgt_constructor_out_<M.D.Y>.tar.gz - a compressed file of the directory `sgt_constructor_out_<M.D.Y>-local` created to ease the movement of all required
    data over to a local machine to render the svg files used in the next step. This is often necessary due to the lack of graphics capabilities of headless servers.
...
\&& - These are standard RAxML output files for each gene that PhyloFisher has appended the
corresponding gene name to. See the [`RAxML documentation`](https://amoeba.msstate.edu/phylofisher/pdfs/raxml.pdf) for a thorough explanation of their
contents.
...
NOTE: If sgt_constructor is circumnavigated to use alternative parameters for sequence filtering, alignment, and tree reconstruction the following criteria must be met to renter the workflow and downstream steps perform correctly:

1. Trees must have been built using a maximum likelihood program.  Downstream quality control steps are not set up to interpret Bayesian
posterior probability values.

2. The directory structure of `sgt_constructor_out_<M.D.Y>-local` outlined above must be replicated.  To do this

    1. Make a directory `my_directory` and within `my_directory` directory make a subdirectory called `tree`
